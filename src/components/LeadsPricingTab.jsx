...
export function LeadsPricingTab({ st, dispatch }) {
  ...
  return (
    <div>
      <Section title="CPL — Cost per Lead" open={open.cpl} onToggle={() => toggle('cpl')}>
        <CPLEditor l={l} LS={LS} LAE={LAE} LAA={LAA} LAD={LAD} LSE={LSE} LSA={LSA} LSD={LSD} />
      </Section>

      <Section title="CPA — Cost per Acquisition" open={open.cpa} onToggle={() => toggle('cpa')}>
        <CPAEditor l={l} LS={LS} LAE={LAE} LAA={LAA} LAD={LAD} LSE={LSE} LSA={LSA} LSD={LSD} />
      </Section>

      <Section title="Hybrid — CPL + CPA" open={open.hyb} onToggle={() => toggle('hyb')}>
        <HybridEditor l={l} LS={LS} LSE={LSE} LSA={LSA} LSD={LSD} />
      </Section>
    </div>
  );
}
