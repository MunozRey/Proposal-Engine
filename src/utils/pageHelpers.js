...
export function logoStr(key, h, st) {
  ...
  if (st[key]) return `<img src="${st[key]}" style="height:${h}px;object-fit:contain;display:block">`;
  return `<span style="font-size:${h * .7}px;font-weight:800;color:#fff">${esc(st.clientName)}</span>`;
}
...
export function ftrStr(st) {
  const left = st?.footerLeft || 'CreditCheck · Clovr Labs S.L. · Confidential';
...
export function tableStr(headers, rows, st) {
  ...
  const trs = rows.map((cells, ri) => {
    ...
    const tds = cells.map(c => `<td style="padding:5px 8px;${c.style || ''}">${c.html}</td>`).join('');
