  ...
  <div style="position:absolute;top:60px;left:42px;right:42px;font-family:inherit">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:18px">
      <div style="background:${ACCENT};border-radius:12px;padding:16px 18px;color:#fff;font-family:${FONTS.SANS}">
        <div style="font-family:${FONTS.MONO};font-size:8.5px;font-weight:700;letter-spacing:0.12em;color:rgba(255,255,255,.92);text-transform:uppercase;margin-bottom:8px">${isEs ? 'Componente 1' : 'Component 1'}</div>
        <div style="font-size:13px;font-weight:600;margin-bottom:6px">${isEs ? 'CPL base reducido' : 'Reduced base CPL'}</div>
        <div style="font-family:${FONTS.SERIF};font-size:30px;font-weight:700;letter-spacing:-0.025em;line-height:1">${esc(l.hybridCPLPrice || '€8')}<span style="font-family:${FONTS.SANS};font-size:11px;font-weight:600;color:rgba(255,255,255,.95)"> / lead</span></div>
        <div style="font-size:9.5px;color:#FFFFFF;margin-top:8px;line-height:1.45">${isEs ? 'Fee fijo en el momento de la entrega del lead verificado.' : 'Fixed fee at the moment of verified lead delivery.'}</div>
      </div>
      <div style="background:${N};border-radius:12px;padding:16px 18px;color:#fff;font-family:${FONTS.SANS}">
        <div style="font-family:${FONTS.MONO};font-size:8.5px;font-weight:700;letter-spacing:0.12em;color:rgba(255,255,255,.92);text-transform:uppercase;margin-bottom:8px">${isEs ? 'Componente 2' : 'Component 2'}</div>
        ...
        <div style="font-size:9.5px;color:#FFFFFF;margin-top:6px">+ <span style="color:${ACCENT};font-weight:700">${esc(l.hybridCPAComm || '0.5%')}</span> ${isEs ? 'sobre importe formalizado' : 'on formalized amount'}</div>
      </div>
    </div>
