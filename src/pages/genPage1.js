...
export function genPage1(st) {
  ...
  const badge = st.coverBadge || (isEs ? 'PROPUESTA DE COLABORACION' : 'PARTNERSHIP PROPOSAL');
  const line1 =
    st.coverLine1 ||
    (isLeads
      ? isEs
  ...
  const line3 =
    st.coverLine3 ||
    (isLeads ? (isEs ? 'para' : 'for') : isEs ? 'white-label para' : 'white-label for');
  ...
  <div style="position:absolute;top:12px;right:14px;font-size:6.5px;color:rgba(255,255,255,.35);letter-spacing:.08em;font-family:inherit">${isEs ? 'CONFIDENCIAL' : 'CONFIDENTIAL'}</div>
  ...
  <div style="position:absolute;top:386px;left:14px;font-family:inherit">
    ...
    <div style="font-size:18px;font-weight:800;color:${B};line-height:1.15">${esc(st.productTitle || (isEs ? 'tu producto o servicio' : 'your product or service'))}</div>
  ...
  <div style="position:absolute;bottom:8px;left:14px;right:14px;display:flex;justify-content:space-between;font-family:inherit">
    <span style="font-size:5.5px;color:rgba(255,255,255,.18)">${esc(st.footerLeft || `${isEs ? 'Tu empresa' : 'Your company'} · ${isEs ? 'Confidencial' : 'Confidential'}`)}</span>
    <span style="font-size:5.5px;color:rgba(255,255,255,.18)">${esc(st.footerRight || (isEs ? 'tu-dominio.com' : 'your-domain.com'))}</span>
  </div></div>`;
}
